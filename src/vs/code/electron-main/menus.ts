...
export class VSCodeMenu {
	...
	private setHelpMenu(helpMenu: Electron.Menu): void {
		...
		arrays.coalesce([
			...
			this.envService.product.requestFeatureUrl ? new MenuItem({ label: mnemonicLabel(nls.localize({ key: 'miUserVoice', comment: ['&& denotes a mnemonic'] }, "&&Search Feature Requests")), click: () => this.openUrl(this.envService.product.requestFeatureUrl, 'openSearchFeatureRequests') }) : null,
